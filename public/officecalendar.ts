let officeHolidays: {[key: string]: MinimalHoliday} = {}

let officeYearsLoaded: {[key: number]: boolean} = {};


function loadOfficeData(month: number, year: number) {
	myfetch('calendar/'+year+'/office').then(res => {
		res.data.forEach((i: HolidayDocument) => officeHolidays[i.year+'-'+i.month+'-'+i.date] = i)
...
async function pageOfficeSetup() {
...
function fillOfficeCalendar(month: number, year: number) {
